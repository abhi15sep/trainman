module.exports = function(plugins, config) {
  ...
  plugins.gulp.task('stylesheets', function() {
    ...
    if (config.deployment && config.environment.deploy.compact) {
  ...
  plugins.gulp.task('assets', function() {
    ...
    return plugins.mergeStream.apply(null, sources).
      pipe(plugins.gulp.dest('public/assets')).pipe(plugins.connect.reload());
  });
  ...
  plugins.gulp.task('templates', function() {
    ...
    return plugins.gulp.
      src('./tasks/templates/**/*.html').
      pipe(plugins.template({
        dependentStylesheetUrls: config.environment.dependencies.stylesheets || [],
        dependentJavascriptUrls: config.environment.dependencies.javascripts || [],
      ...
      pipe(plugins.gulp.dest('public')).pipe(plugins.connect.reload());
  });

  plugins.gulp.task('clean', function(callback) {
    plugins.del(['./public/index.html', './public/assets/**/*'], function(err) {
      callback(err);
    });
  });
