...
module.exports = function(plugins, config) {
  ...
  plugins.gulp.task('deploy', ['build'], function() {
    ...
    var indexSource = indexOutputFile ? path.join(config.publicDir, config.indexOutputFile) : null,
        otherSources = [path.join(publicDir, '/**')];

    if (indexSource) {
      otherSources.push('!' + path.join(config.publicDir, config.indexOutputFile));
    }
