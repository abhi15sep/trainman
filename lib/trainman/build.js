...
module.exports = function( plugins, config, options ) {
  ...
  plugins.gulp.task(
    ...
    function( callback ) {
      ...
      stylesheetUrls.push( dependencies.stylesheets || [] );
      stylesheetUrls.push( root + assetBasename + '.css' );

      javascriptUrls.push( dependencies.javascripts || [] );
      ...
      for ( var i = 0; i < sources.length; i++ ) {
        ...
        for ( var j = 0; j < source.files.length; j++ ) {
          ...
          if ( config.cacheTemplates && !isIndex ) {
            stream = stream.
              pipe( plugins.angularTemplatecache( {
                transformUrl: function( url ) {
                  url = url.replace( cwdRegex, '' );
                  url = path.join( config.assetOutputPath, url );
