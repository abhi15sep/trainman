...
module.exports = function( plugins, config, options ) {
  ...
  plugins.gulp.task(
    ...
    function( callback ) {
      ...
      // Template Filters
      var allButIndex = filter(['**', '!app/templates/index.html'], {restore: true});
      var index = filter(['*', 'app/templates/index.html'], {restore: true});

      var mainStream = queueStreams( sources ).
                          pipe( plugins.template( locals ) );

      var indexStream = mainStream.
                          pipe( index ).
      ...
      var otherStreams = mainStream.
                          pipe( allButIndex ).
                          pipe( plugins.gulp.dest( config.publicDir + '/assets/templates' ) ).
                          pipe( print( function(filepath) {
                            return "path: " + filepath;
                          }));
                          // pipe( plugins.connect.reload() );

      var mergedStream = merge( indexStream, otherStreams ).
                          pipe( print( function(filepath) {
