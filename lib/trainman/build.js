...
module.exports = function( plugins, config, options ) {
  ...
  plugins.gulp.task(
    ...
    function( callback ) {
      ...
      if ( sources.length < 1 ) callback();

      for ( var i = 0; i < sources.length; i++ ) {
        ( function( localIndex ) {
        sortedSources[ localIndex ] = plugins.gulp.
          src( sources[ localIndex ].files, { base: sources[ localIndex ].base } ).
          pipe(print(function(filepath) {
            return "css-presorted" + localIndex + " " + filepath;
          })).
          pipe( plugins.order( sources[ localIndex ].files, { base: '.' } ) ).
          pipe(print(function(filepath) {
            return "css-postsorted" + localIndex + " " + filepath;
          }));
        } )( i );
      }
      ...
      pipe(print(function(filepath) {
        return "final-css " + filepath;
      })).
        pipe( plugins.sass().on( 'error', plugins.sass.logError ) ).
        pipe( plugins.concat( assetBasename + '.css' ) ).
        pipe(print(function(filepath) {
          return "final-css-2 " + filepath;
        }));

      if ( config.compact ) {
        task = task.pipe( plugins.cleanCss() );
      }
    ...
    function( callback ) {
      ...
      for ( var i = 0; i < sources.length; i++ ) {
        ...
        sortedSources[ localIndex ] = plugins.gulp.
          src( sources[ localIndex ].files, { base: sources[ localIndex ].base } ).
          pipe(print(function(filepath) {
            return "js-presort" + localIndex + " " + filepath;
          })).
          pipe( plugins.order( sources[ localIndex ].files, { base: '.' } ) ).
          pipe(print(function(filepath) {
            return "js-postsort" + localIndex + " " + filepath;
          }));
        } )( i );
      }
