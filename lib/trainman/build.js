...
module.exports = function( plugins, config, options ) {
  ...
  plugins.gulp.task(
    ...
    function( callback ) {
      ...
      // var streams = queueStreams( sources, true );
      var streams = [],
          source;
      ...
      for ( var i = 0; i < sources.length; i++ ) {
        ...
        for ( var j = 0; j < source.files.length; j++ ) {
          ...
          // stream = plugins.gulp.src( file, { base: source.base } );
          // stream = plugins.gulp.src( file, { base: source.base } ).pipe(print(function(v){return 'NAME: '+v;}));
          // stream = plugins.gulp.src( file, { base: source.base } ).pipe(debug({title: 'unicorn:', minimal:true}));
