'use strict';

var gulp = require( 'gulp' ),
    runSequence = require( 'run-sequence' );

var clean = require( './clean' ),
    connect = require( 'gulp-connect' ),
    stylesheets = require( './build/stylesheets' ),
    javascripts = require( './build/javascripts' ),
    assets = require( './build/assets' ),
    templates = require( './build/templates' ),
    log = require( './utils' ).log;

module.exports = function( config, express ) {
  if ( express ) {
    return function() {
      clean( config ).then( function() {
        log( 'build' );

        stylesheets( config );
        javascripts( config );
        assets( config );
        templates( config );
      } );
    }
  }
  else {
    gulp.task(
      'stylesheets',
      function( callback ) {
        return stylesheets( config, true, callback ).pipe( connect.reload() );
      }
    );

    gulp.task(
      'javascripts',
      function( callback ) {
        return javascripts( config, true, callback ).pipe( connect.reload() );
      }
    );

    gulp.task(
      'assets',
      function( callback ) {
        return assets( config, true, callback ).pipe( connect.reload() );
      }
    );

    gulp.task(
      'templates',
      function( callback ) {
        return templates( config, true, callback ).pipe( connect.reload() );
      }
    );

    gulp.task(
      'clean',
      function() {
        return clean( config, true );
      }
    );

    gulp.task( 'build', [ 'clean' ], function( callback ) {
      runSequence( [ 'stylesheets', 'javascripts', 'assets', 'templates' ], callback );
    } );
  }
};
