...
module.exports = function( projectConfig, environment ) {
  ...
  config.variantOverride = options.variant || options.v;
  ...
  return {
    ...
    getThemeVariant: function( hostname ) {
      var settingsEndpoint = url.parse( config.environment.metaTags[ 'com.boxxspring.api.url' ] );
      settingsEndpoint.protocol = settingsEndpoint.protocol || 'http:';
      settingsEndpoint = url.format( settingsEndpoint ) + '/properties?include[settings]=true';

      if ( config.codeNameOverride ) {
        settingsEndpoint += '&code_name=' + config.codeNameOverride;
      }
      else {
        settingsEndpoint += '&domain_name=' + hostname;
      }

      return {
        then: function( success, failure ) {
          requestPromise( settingsEndpoint ).then(
            function( resultsJSON ) {
              ...
              if ( results.settings ) {
                for (var i = 0; i < results.settings.length; i++) {
                  setting = results.settings[ i ];

                  if ( setting.name === 'com.boxxspring.theme.variant' ) {
                    variant = setting.content;
                    break;
                  }
                }
              }
